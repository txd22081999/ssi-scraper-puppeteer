...
const VIBER_BOT_URL = 'http://localhost:5080/hpg'
...
const parseSSI = (raw) => {
  ...
  //   ).strftime("%X")
  const data = raw['response']['payloadData'].split('|')

  const dp = {
    // timestamp2: raw['data']['timestamp'],
    timestamp: raw['timestamp'],
    time: moment(new Date(parseInt(raw['timestamp']) + 7 * 1000 * 3600)).format(
      'HH:mm:ss'
    ),
    hoseId: data[0],
    code: data[1],
...
const scrapePageHose = async (page) => {
  ...
  session.on('Network.webSocketFrameReceived', (raw) => {
    // console.log(
    //   '==================================================================== RECEIVED HOSE ===================================================================='
    // )
    const outputPath = `/Users/mac/Workspaces/Code/ps-indicator/ssi-scraper/hpg/output.txt`

    const rawStr = JSON.stringify(raw)
    if (!rawStr.includes('#hose:1354')) {
      return
    }

    console.log('received HPG', JSON.stringify(raw))
    const hpg = parseSSI(raw)
    if (hpg.bestBid1 === 0 || hpg.bestOffer1 === 0) return
    if (HPG.length > 0) {
      const lastHPG = HPG[HPG.length - 1]
      if (
        hpg.bestBid1 !== lastHPG.bestBid1 ||
        hpg.bestOffer1 !== lastHPG.bestOffer1
      ) {
        HPG.push(hpg)
        if (HPG.length === 3) {
          HPG.forEach((dp, index) => {
            fs.appendFile(outputPath, JSON.stringify(dp), function (err) {
              if (err) throw err
            })
            axios.post(VIBER_BOT_URL, {
              message: JSON.stringify({ index, ...dp }),
            })
          })
          console.log(HPG)
          HPG.pop()
