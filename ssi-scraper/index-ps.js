...
function sendDataAPI(data) {
    ...
    console.log(data)
    postData(data).then(console.log).then((data) => data);
}
...
;(async () => {
  ...
  try {
    ...
    const URL = 'https://iboard.ssi.com.vn/bang-gia/phai-sinh'
    await page.goto(URL)
    ...
    session.on('Network.webSocketFrameSent', (x) => {
      ...
      console.log(`Date: ${dateObj.toDateString()}`)
           if(!IGNORE_KEY_SCRAPER.some(key => x.response.payloadData.includes(key))){
         console.log("Sent data API")
         sendDataAPI({
